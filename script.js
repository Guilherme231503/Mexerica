...
const colorNames = {
    'cinza': 'Cinza-do-Pecado',
    'laranja': 'Laranja-Nordeste',
...
function activateCarouselView(color) {
    ...
    gsap.to([frontSlideImg, backSlideImg], {
        ...
        duration: 0.2,
        onComplete: () => {
            frontSlideImg.attr('src', `mexerica_phone_${color}_front.png`);
            backSlideImg.attr('src', `mexerica_phone_${color}_back.png`);
            
            gsap.to([frontSlideImg, backSlideImg], { opacity: 1, duration: 0.2 });

            // 3. ATUALIZAÇÃO DO VERSO NA SEÇÃO DE DETALHES
            backDetailImg.src = `mexerica_phone_${color}_back_full.png`;
...
function filterOptionsByModel() {
    ...
    if (modelType !== "promax") {
        ...
        memoryOptions.forEach(opt => {
            if (opt.getAttribute('data-memory') === "8") {
...
$('#add-to-cart').on('click', function() {
   ...
   if(selectedModel == 'pro') {
    ...
    localStorage.setItem('mexericaPhoneOrder', JSON.stringify(orderDetails));
